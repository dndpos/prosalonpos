  ...
  return(
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.6)',zIndex:500,display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'Inter',system-ui,sans-serif"}} onClick={onClose}>
      <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:10}} onClick={e=>e.stopPropagation()}>
        ...
        {/* Customer / Store / Tech toggle */}
        <div style={{display:'flex',gap:4,background:C.chromeDark,borderRadius:8,padding:3}}>
          {[{id:'customer',label:'Customer'},{id:'store',label:'Store'},{id:'tech',label:'Tech Copy'}].map(t=>{
            const active=copyType===t.id;
            return(<button key={t.id} onClick={()=>setCopyType(t.id)}
              style={{height:32,padding:'0 16px',background:active?C.blue:'transparent',border:'none',borderRadius:6,color:active?'#fff':C.textMuted,fontSize:12,fontWeight:active?600:400,cursor:'pointer',fontFamily:'inherit'}}>{t.label}</button>);
          })}
        </div>
              ...
              return (
                <div key={group.techId} style={{width:W,background:'#FEFEF6',borderRadius:4,padding:'16px 18px',boxShadow:'0 4px 20px rgba(0,0,0,0.4)',...base}}>
                  ...
                  <div style={{...center,marginBottom:6}}>
                    <div style={{fontSize:13,fontWeight:700,letterSpacing:'0.05em'}}>TICKET #{ticket.ticketNumber}</div>
                  </div>
                  {sep}
                  {/* Tech name */}
        ...
        <div style={{width:W,maxHeight:'80vh',overflow:'auto',background:'#FEFEF6',borderRadius:4,padding:'20px 18px',boxShadow:'0 8px 40px rgba(0,0,0,0.5)',...base}}>
          ...
          <div style={{marginBottom:2}}>
            ...
            {ticket.itemDiscTotal>0 && <div style={row}><span>Item discounts</span><span>-{fmt(ticket.itemDiscTotal)}</span></div>}
        ...
        <button onClick={onClose}
          style={{height:40,padding:'0 28px',background:C.chrome,border:`1px solid ${C.borderMedium}`,borderRadius:8,color:C.textPrimary,fontSize:13,fontWeight:500,cursor:'pointer',fontFamily:"'Inter',system-ui,sans-serif"}}>
          Close Preview
        </button>
      </div>
    </div>
