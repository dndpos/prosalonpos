...
export default function ClientList({ onBack }) {
  ...
  if (selectedClient) {
    ...
    return <ClientProfileScreen client={fresh} onBack={handleBack} onBackToCalendar={onBack} onUpdateClient={handleUpdateClient} />;
  }
  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', background: C.chrome, fontFamily: F, overflow: 'hidden', position: 'relative' }}>
      ...
      <div style={{ height: 52, background: C.chromeDark, display: 'flex', alignItems: 'center', padding: '0 20px', gap: 12, borderBottom: '1px solid ' + C.borderLight, flexShrink: 0 }}>
        {onBack && <div onClick={onBack} style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#0E2E3D', border: '1px solid #1A4A5C', color: '#7EC8E3', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', padding: '7px 16px', borderRadius: 6, minHeight: 36, userSelect: 'none', transition: 'background 150ms' }}
