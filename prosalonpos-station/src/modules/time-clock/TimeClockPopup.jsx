...
export default function TimeClockPopup({ show, clockPunches, presenceRecords, onPunch, onPresencePunch, onDismiss }) {
  ...
  function getClockStatus(staffId) {
    ...
    // Presence records are the source of truth for "who is currently here"
    // for ALL staff types. Clock punches are for payroll/timesheets only.
    var rec = (presenceRecords || []).find(function(r) { return r.staff_id === staffId; });

    if (isHourly) {
      // For hourly, also check clock punches as fallback if no presence record yet
      // (handles legacy data before presence records were created for hourly staff)
      if (rec) {
      ...
      // Non-hourly: presence records only
  ...
  function handleConfirm() {
    ...
    if (isHourly) {
      ...
      if (onPresencePunch) onPresencePunch(staff.id, confirmTarget.punchType === 'in' ? 'in' : 'out');
    } else {
