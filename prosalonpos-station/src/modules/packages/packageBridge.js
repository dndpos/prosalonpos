...
export function getClientPackages(clientId) {
  return MOCK_CLIENT_PACKAGES.filter(function(cp) {
...
export function findRedeemablePackageItems(clientId, ticketServiceId, allServices, allCategories) {
  ...
  clientPkgs.forEach(function(cp) {
    ...
    items.forEach(function(cpi) {
      ...
      var pkgService = allServices.find(function(s) { return s.id === cpi.service_id; });
      if (!pkgService) return;
      var pkgCategoryIds = pkgService.category_ids || [];

      var sharedCategory = pkgCategoryIds.some(function(catId) {
        return ticketCategoryIds.indexOf(catId) !== -1;
      });

      if (sharedCategory) {
        var diff = (ticketService.price_cents || 0) - (pkgService.price_cents || 0);
        results.push({ clientPackage: cp, clientPackageItem: cpi, isExactMatch: false, upgradeDifferenceCents: Math.max(0, diff) });
      }
    });
  });
