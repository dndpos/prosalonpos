...
export default function useGridLayoutState(salonSettings, handleSettingsUpdate, gridPersist) {
  ...
  var [empColumns, setEmpColumns] = useState(6);
  var [empRows, setEmpRows] = useState(6);
  ...
  useEffect(function() {
    ...
    if (storeStaff.length > 0) {
      ...
      setEmpSlots(function(prev) {
        ...
        var slots = {};
        storeStaff.filter(function(s) { return s.active; })
          .sort(function(a, b) { return (a.display_name || a.name || '').localeCompare(b.display_name || b.name || ''); })
          .forEach(function(emp, i) { slots[i] = emp.id; });
        return slots;
      });
    }
  }, [storeStaff]);
