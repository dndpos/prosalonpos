...
export default function App() {
  ...
  useEffect(function() {
    ...
    // Debounce ticket socket events — same pattern as appointments.
    // Merge-and-close fires socket event that races with local optimistic update,
    // causing ticket list badge flash (TD-150). 1s debounce lets optimistic state settle.
    var _ticketDebounce = null;
    function debouncedFetchTickets() {
      if (_ticketDebounce) clearTimeout(_ticketDebounce);
      _ticketDebounce = setTimeout(function() { fetchTickets(); }, 1000);
    }
    onSocketEvent('ticket:created', debouncedFetchTickets);
    onSocketEvent('ticket:updated', debouncedFetchTickets);
    onSocketEvent('ticket:closed', debouncedFetchTickets);
    onSocketEvent('ticket:voided', debouncedFetchTickets);
    onSocketEvent('ticket:refunded', function() { debouncedFetchTickets(); fetchGiftCards(); });
    onSocketEvent('ticket:payment', debouncedFetchTickets);
    onSocketEvent('ticket:tip_updated', debouncedFetchTickets);
  ...
  function handleOpenTicketCheckout(ticketIds) {
    ...
    if (selected.length >= 2) {
      // Combine items client-side — NO server merge yet.
      // Merge happens at payment time in handleCloseTicket.
      // Sort by ticket_number so lowest is the "absorber"
      selected.sort(function(a, b) { return (a.ticket_number || 0) - (b.ticket_number || 0); });
      var primary = selected[0];
      var allItems = [];
      selected.forEach(function(t) { allItems = allItems.concat(t.items || []); });
      var numbers = selected.map(function(t) { return t.ticket_number; });
      var displayNum = numbers.join('&');
      var totalDeposit = selected.reduce(function(sum, t) { return sum + (t.depositCents || t.deposit_cents || 0); }, 0);
      // Collect all appointment IDs from the tickets being combined
      var apptIds = selected.map(function(t) { return t.appointmentId || t.appointment_id; }).filter(Boolean);
      var enrichedClient = enrichClientBalance(primary.client_id ? { id: primary.client_id, name: primary.clientName } : primary.client);
      setCheckoutData({
        ...
        pendingMerge: true,
        appointmentId: apptIds[0] || null,
        allAppointmentIds: apptIds.length > 1 ? apptIds : null,
      });
      setActivePage('checkout');
    } else {
      ...
      setCheckoutData({
        ...
        appointmentId: primary.appointmentId || primary.appointment_id || null,
      });
      setActivePage('checkout');
    }
  }
  ...
  // renderOtherPage: renders NON-calendar pages. Calendar is always mounted separately.
  ...
  return (
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', width: '100vw', backgroundColor: C.bg, overflow: 'hidden' }}>
      ...
      <main style={{ flex: 1, overflow: 'hidden', position: 'relative' }}>
        {/* Calendar is ALWAYS mounted — hidden via CSS when another page is active. 
            This prevents the remount→refetch→flash cycle that caused the grid flicker. */}
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: activePage === 'calendar' ? 'block' : 'none' }}>
        ...
        {activePage !== 'calendar' && renderOtherPage()}
      </main>
