...
// ─── Socket ID for broadcast exclusion ───
// Server uses this to exclude the sender from socket broadcasts,
// preventing redundant refetches that cause UI flicker.
var _socketId = null;
function setSocketId(id) { _socketId = id; }
function getSocketId() { return _socketId; }
...
async function request(method, path, body) {
  ...
  if (_elapsed > 500) {
    console.warn('[API SLOW] ' + method + ' ' + path + ' took ' + _elapsed + 'ms (limit: 500ms)');
  }
