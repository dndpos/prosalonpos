...
export default function useTicketHandlers() {
  ...
  var storeMergeTickets = useTicketStore(function(s) { return s.mergeTickets; });
  var storeMergeAndClose = useTicketStore(function(s) { return s.mergeAndClose; });
  // (storeUpdateServiceLine and storeUpdateAppointment removed C10 — server handles persistence,
  //  client does local-only optimistic updates to avoid redundant API round-trips)
  ...
  function handleCloseTicket(ticket) {
    ...
    // For pendingMerge: remove all ticket IDs immediately so they vanish in one shot
    if (ticket.openTicketIds && ticket.openTicketIds.length > 0) {
      ticket.openTicketIds.forEach(function(tid) {
        storeRemoveOpenTicket(tid);
      });
    }
    // Connection 3: mark originating calendar service lines as checked_out (LOCAL ONLY)
    // Server already marks appointments + service lines in the close/merge transaction.
    // We only update local state here for instant visual feedback, then do ONE fetch.
    var payMethod = (ticket.payments && ticket.payments[0]) ? ticket.payments[0].method : (ticket.packageRedemptions ? 'package' : null);
    var apptIdsToMark = ticket.allAppointmentIds || (ticket.appointmentId ? [ticket.appointmentId] : []);
    var apptStore = useAppointmentStore.getState();
    // Collect all service line IDs to mark
    var slIdsToMark = [];
    if (ticket.serviceLineIds && ticket.serviceLineIds.length > 0) {
      slIdsToMark = ticket.serviceLineIds.slice();
    } else {
      apptIdsToMark.forEach(function(aId) {
        var lines = apptStore.getServiceLinesForAppointment(aId);
        lines.forEach(function(sl) { slIdsToMark.push(sl.id); });
      });
    }
    // Optimistic local update — no API calls, just update the store directly
    if (slIdsToMark.length > 0 || apptIdsToMark.length > 0) {
      useAppointmentStore.setState(function(s) {
        var updated = s.serviceLines.map(function(sl) {
          if (slIdsToMark.indexOf(sl.id) >= 0) {
            return Object.assign({}, sl, { status: 'checked_out', payment_method: payMethod });
          }
          return sl;
        });
        return { serviceLines: updated };
      });
    }
    ...
    if (storeSource === 'api') {
      ...
      function addPaymentsAndClose(dbTicketId) {
        ...
        Promise.all(paymentPromises).then(function() {
          ...
          // Fetch service lines so calendar shows paid badge (server already marked appointment)
          var ld = useAppointmentStore.getState().loadedDate;
          if (ld) useAppointmentStore.getState().fetchServiceLines(ld);
        }).catch(function(err) {
      ...
      if (needsCreate) {
        ...
        // Ticket already exists in DB (came from calendar or hold)
        // If pendingMerge: merge tickets on server FIRST, then close the absorber
        var pendingMerge = ticket.openTicketIds && ticket.openTicketIds.length >= 2;
        if (pendingMerge) {
          // Atomic merge + pay + close in one server call — no intermediate flashes
          var mergePayments = (ticket.payments || []).map(function(p) {
            return {
              method: p.method || 'credit',
              amount_cents: p.amount_cents || p.amountCents || 0,
              gc_id: p.gc_id || null,
          ...
          storeMergeAndClose(ticket.openTicketIds, mergePayments, mergeCloseData).then(function(closedTicket) {
            redeemPackagesForTicket(closedTicket.id);
            createGiftCardsForTicket(closedTicket.id);
            redeemGiftCardPayments(closedTicket.id);
            // Fetch service lines so calendar shows paid badges (server already marked all appointments)
            var ld = useAppointmentStore.getState().loadedDate;
            if (ld) useAppointmentStore.getState().fetchServiceLines(ld);
          }).catch(function(err) {
            console.error('[handleCloseTicket] Merge-and-close FAILED:', err.message);
            alert('⚠ Ticket merge failed: ' + (err.message || 'Server error'));
            setCheckoutError({ message: 'Merge-and-close failed: ' + (err.message || 'Server error'), ticketNumber: ticket.ticketNumber, ts: Date.now() });
          });
        } else {
          // Single ticket — update items then close
          var updatedItems = (ticket.items || []).map(function(it) {
            return {
              type: it.type || 'service',
              name: it.name || 'Service',
