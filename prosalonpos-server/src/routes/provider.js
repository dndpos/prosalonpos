...
router.put('/salons/:id/status', async function(req, res, next) {
  try {
    ...
    if (!['active', 'trial', 'suspended'].includes(newStatus)) {
      return res.status(400).json({ error: 'Invalid status. Allowed: active, trial, suspended' });
    }
...
// ── DELETE /salons/:id — Permanently delete salon and ALL related data ──
router.delete('/salons/:id', requireOwner, async function(req, res, next) {
  try {
    var salonId = req.params.id;
    var salon = await prisma.salon.findUnique({ where: { id: salonId } });
    if (!salon) return res.status(404).json({ error: 'Salon not found' });

    var salonName = salon.name;
    console.log('[Provider] CASCADE DELETE starting for salon:', salonName, '(' + salonId + ')');

    // Use interactive transaction for sequential execution (array syntax runs parallel → FK violations)
    await prisma.$transaction(async function(tx) {
      // Delete in dependency order — children first, parent last
      // Deepest children
      await tx.$executeRawUnsafe('DELETE FROM "PackageRedemption" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "ClientPackageItem" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "ClientPackage" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "ServicePackageItem" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "ServicePackage" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "LoyaltyTransaction" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "LoyaltyAccount" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "LoyaltyReward" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "LoyaltyTier" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "LoyaltyProgram" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "GiftCardTransaction" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "GiftCard" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "MembershipAccount" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "MembershipPerk" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "MembershipPlan" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "MessageLogEntry" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "MessageTemplate" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "PunchAuditLog" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "ClockPunch" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "StaffPresence" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "CommissionTier" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "CommissionRule" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "TicketPayment" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "TicketItem" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "Ticket" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "ServiceLine" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "BlockedTime" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "Appointment" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "Client" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "Product" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "ProductCategory" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "Supplier" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "ServiceStaffAssignment" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "CategoryStaffAssignment" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "ServiceCatalogCategory" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "ServiceCatalog" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "ServiceCategory" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "SalonSettings" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "Staff" WHERE salon_id = $1', salonId);
      // Provider records tied to this salon
      await tx.$executeRawUnsafe('DELETE FROM "ProviderSalonNote" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "ProviderBillingRecord" WHERE salon_id = $1', salonId);
      await tx.$executeRawUnsafe('DELETE FROM "ProviderAuditLog" WHERE salon_id = $1', salonId);
      // Finally the salon itself
      await tx.$executeRawUnsafe('DELETE FROM "Salon" WHERE id = $1', salonId);
    }, { timeout: 30000 });

    console.log('[Provider] CASCADE DELETE complete for salon:', salonName);
    await addAudit(req, 'salon_deleted', 'Permanently deleted salon: ' + salonName + ' (' + salonId + ')', null);
    res.json({ success: true, message: 'Salon "' + salonName + '" permanently deleted' });
  } catch (err) {
    console.error('[Provider] CASCADE DELETE failed:', err);
    next(err);
  }
});
