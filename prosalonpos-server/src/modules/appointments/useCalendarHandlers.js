...
export default function useCalendarHandlers(ctx) {
  ...
  function handleBookingSave(data) {
    ...
    var bookingLineIds = newLines.map(function(l) { return l.id; });
    persist.saveBooking(newLines).then(function() {
      // Refetch from server to replace sl-book- IDs with real server IDs that have appointment_id
      var d = ctx.selectedDate;
      if (d && ctx.fetchServiceLines) {
        var dateStr = d.getFullYear() + '-' + String(d.getMonth()+1).padStart(2,'0') + '-' + String(d.getDate()).padStart(2,'0');
        ctx.fetchServiceLines(dateStr).then(function() {
          // Bypass optimistic lock — directly set local state from store
          var storeLines = useAppointmentStore.getState().serviceLines;
          if (ctx.setServiceLinesDirect) ctx.setServiceLinesDirect(storeLines);
        });
      }
