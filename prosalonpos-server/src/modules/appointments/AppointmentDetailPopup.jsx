 ...
 * Config constants extracted to appointmentDetailConfig.js (V3).
 */
import { useState } from 'react';
import { useClientStore } from '../../lib/stores/clientStore';
import AreaTag from '../../components/ui/AreaTag';
import {
  STATUS_CONFIG, ALLOWED_TRANSITIONS, TRANSITION_LABELS, TRANSITION_CONFIRM_MSG,
  TRANSITION_STYLES, EXTRA_TIME_OPTIONS, SERVICE_CATEGORIES, SERVICE_CATALOG,
  formatTimeFull, formatTimeShort, LockIcon, TechAvatar,
} from './appointmentDetailConfig.jsx';

// Re-export for external use
export { STATUS_CONFIG, ALLOWED_TRANSITIONS };
...
export default function AppointmentDetailPopup({sl, allServiceLines, staff, onClose, onStatusChange, onChangeTech, onAddTime, onAddService, onCheckout}){
  ...
  // Must filter by appointment_id to avoid pulling lines from other appointments for the same client
  var relatedLines;
  if (sl.appointment_id) {
    relatedLines = allServiceLines.filter(function(s) { return s.appointment_id === sl.appointment_id && s.id !== sl.id; });
  } else {
    // Fallback: no appointment_id — group by client + time window (contiguous/overlapping only)
    // Find lines for same client that overlap or are within 5 min of this appointment's time range
    var slStart = sl.starts_at instanceof Date ? sl.starts_at.getTime() : new Date(sl.starts_at).getTime();
    var slEnd = slStart + (sl.dur || 30) * 60000;
    var candidates = allServiceLines.filter(function(s) { return s.client === sl.client && s.id !== sl.id; });
    // Iteratively expand the window to include contiguous lines
    var grouped = [sl];
    var changed = true;
    while (changed) {
      changed = false;
      var gStart = Math.min.apply(null, grouped.map(function(g) { return g.starts_at instanceof Date ? g.starts_at.getTime() : new Date(g.starts_at).getTime(); }));
      var gEnd = Math.max.apply(null, grouped.map(function(g) { var t = g.starts_at instanceof Date ? g.starts_at.getTime() : new Date(g.starts_at).getTime(); return t + (g.dur || 30) * 60000; }));
      candidates.forEach(function(c) {
        if (grouped.some(function(g) { return g.id === c.id; })) return;
        var cStart = c.starts_at instanceof Date ? c.starts_at.getTime() : new Date(c.starts_at).getTime();
        var cEnd = cStart + (c.dur || 30) * 60000;
        // Within 5 min of group boundary = contiguous
        if (cStart <= gEnd + 300000 && cEnd >= gStart - 300000) {
          grouped.push(c);
          changed = true;
        }
      });
    }
    relatedLines = grouped.filter(function(g) { return g.id !== sl.id; });
  }
  relatedLines.sort(function(a, b) { return a.starts_at - b.starts_at; });
