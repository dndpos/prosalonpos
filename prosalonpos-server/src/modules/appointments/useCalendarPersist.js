...
function useCalendarPersist(toast) {
  ...
  function saveStatus(sl, newStatus) {
    ...
    // Prefer updating via appointment (cascades to all service lines)
    ...
    // Fallback: appointment_id missing (sl-book- race). Update via service line directly.
    // Server will cascade status to parent appointment.
    var isRealId = sl.id && sl.id.indexOf('sl-book-') !== 0 && sl.id.indexOf('sl-') !== 0;
    if (isRealId) {
      return api.put('/appointments/service-line/' + sl.id, { status: newStatus }).catch(function(err) {
        showError('save status (via service line)', err);
        throw err;
      });
    }
    // No real ID at all — nothing to persist
