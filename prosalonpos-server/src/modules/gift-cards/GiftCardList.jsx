...
export default function GiftCardList() {
  ...
  function handleLookup() {
    if (lookupCode.length < 1) return;
    var lookupFn = useGiftCardStore.getState().lookupGiftCard;
    lookupFn(lookupCode.trim()).then(function(card) {
    ...
    function onKey(e) {
      ...
      if (e.key >= '0' && e.key <= '9') {
        ...
        setLookupCode(function(prev) {
          if (prev.length >= 1) {
            var lookupFn = useGiftCardStore.getState().lookupGiftCard;
            lookupFn(prev.trim()).then(function(card) {
              setLookupResult(card || 'not_found');
            }).catch(function(err) {
              console.warn('[GC Lookup] API error:', err.message);
              setLookupResult('not_found');
        ...
        <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
          ...
          <div style={{ background: T.bg, border: '1px solid ' + T.border, borderRadius: 8, padding: 8, flexShrink: 0 }}>
            ...
            <div
              ...
              onMouseEnter={function(e) { if (lookupCode.length >= 1) e.currentTarget.style.backgroundColor = '#1D4FD7'; }}
              ...
              style={{
                ...
                borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: lookupCode.length >= 1 ? 'pointer' : 'default',
                backgroundColor: T.primary, color: '#fff', border: '1px solid ' + T.primary,
                opacity: lookupCode.length >= 1 ? 1 : 0.4, userSelect: 'none',
                transition: 'background-color 150ms',
              }}
            >Check balance</div>
          </div>
