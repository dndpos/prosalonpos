...
export var CHECKOUT_SETTINGS = new Proxy(_settingsDefaults, {
  get: function(defaults, key) {
    ...
    // tax_rate_percentage: settings UI saves as tax_rate_percent — prefer that
    if (key === 'tax_rate_percentage') {
      if (s.tax_rate_percent !== undefined) return s.tax_rate_percent;
      if (s.tax_rate_percentage !== undefined) return s.tax_rate_percentage;
      return defaults.tax_rate_percentage;
    }
...
function _buildCheckoutStaff() {
  return _getStaff().filter(function(s) { return s.active && s.status !== 'deleted' && s.show_on_calendar !== false; });
}
...
function _buildRetailItems() {
  ...
  return products.filter(function(p) { return p.active; }).map(function(p) {
    ...
    return { id: p.id, name: p.name, price_cents: p.price_cents, cat: cat ? cat.name : 'Other', category_id: p.category_id || null, barcode: p.sku || '' };
  });
}

function _buildRetailCategories() {
  var cats = _getInventoryCategories();
  var activeCats = cats.filter(function(c) { return c.active; }).sort(function(a, b) { return (a.position || 0) - (b.position || 0); });
