...
import { Card, SectionTitle, FieldRow, Toggle, Select, Input, handlePriceKey, payNumpad } from './SalonSettingsShared.jsx';
import AreaTag from '../../components/ui/AreaTag';
/**
 * SalonSettingsPanel — All salon settings accordion sections
 * Shared form components extracted to SalonSettingsShared.jsx (V3)
 */
const ROTATION_OPTS = [
...
// Shared form components imported from SalonSettingsShared.jsx
    ...
    <div style={{ height: '100%' }}>
      ...
      <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <AccSection id="checkout" title="Checkout, Tax, Discount & Tips" {...accProps}>
            ...
            <FieldRow label="Tax on services" desc="Charge sales tax on service items (haircuts, color, etc.)" T={T}><Toggle value={settings.tax_on_services !== false} onChange={function(v) { set('tax_on_services', v); }} T={T} /></FieldRow>
            <FieldRow label="Tax on products" desc="Charge sales tax on retail products and inventory items" T={T}><Toggle value={settings.tax_on_products !== false} onChange={function(v) { set('tax_on_products', v); }} T={T} /></FieldRow>
            <FieldRow label="Tax on gift cards" desc="Charge sales tax when a gift card is purchased (most states: OFF)" T={T}><Toggle value={!!settings.tax_on_giftcards} onChange={function(v) { set('tax_on_giftcards', v); }} T={T} /></FieldRow>
            <FieldRow label="Tax on packages" desc="Charge sales tax on package purchases" T={T}><Toggle value={settings.tax_on_packages !== false} onChange={function(v) { set('tax_on_packages', v); }} T={T} /></FieldRow>
            <FieldRow label="Tax on memberships" desc="Charge sales tax on membership sign-ups and renewals" T={T}><Toggle value={settings.tax_on_memberships !== false} onChange={function(v) { set('tax_on_memberships', v); }} T={T} /></FieldRow>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <AccSection id="advcomm" title="Advanced Commission Setup" {...accProps}>
            ...
            <FieldRow label="Minimum daily hours for commission" desc="Tech must work at least this many hours in a day to earn commission on that day's tickets. Leave blank or 0 for no minimum." T={T}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                <Input value={settings.min_daily_hours_for_commission || ''} onChange={function(v) { set('min_daily_hours_for_commission', v === '' ? '' : parseFloat(v) || 0); }} type="number" style={{ width: 70 }} T={T} />
                <span style={{ color: T.textMuted, fontSize: 13 }}>hrs</span>
              </div>
            </FieldRow>
          </AccSection>
