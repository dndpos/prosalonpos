...
export default function DateRangePicker({ show, pickStart, pickEnd, setPickStart, setPickEnd, leftMonth, rightMonth, setLeftMonth, setRightMonth, onApply, onClose }) {
  ...
  function shiftYear(side, dir) {
    var setter = side === 'left' ? setLeftMonth : setRightMonth;
    var cur = side === 'left' ? leftMonth : rightMonth;
    setter({ year: cur.year + dir, month: cur.month });
  }
  ...
  function renderCalendar(mo, side) {
    ...
    return (
      <div style={{ flex: 1, minWidth: 280 }}>
        {/* Month header with year + month arrows */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, padding: '0 4px' }}>
          <div style={{ display: 'flex', gap: 4 }}>
            <div onClick={function() { shiftYear(side, -1); }}
              style={{ width: 36, height: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 6, background: T.grid, cursor: 'pointer', fontSize: 14, color: T.textMuted, fontWeight: 600, userSelect: 'none' }}
              onMouseEnter={function(e) { e.currentTarget.style.background = T.gridHover; }}
              onMouseLeave={function(e) { e.currentTarget.style.background = T.grid; }}>««</div>
            <div onClick={function() { shiftMonth(side, -1); }}
              style={{ width: 36, height: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 6, background: T.grid, cursor: 'pointer', fontSize: 20, color: T.text, fontWeight: 600, userSelect: 'none' }}
              onMouseEnter={function(e) { e.currentTarget.style.background = T.gridHover; }}
              onMouseLeave={function(e) { e.currentTarget.style.background = T.grid; }}>‹</div>
          </div>
          <div style={{ color: T.text, fontSize: 16, fontWeight: 600 }}>{MONTH_NAMES[month]} {year}</div>
          <div style={{ display: 'flex', gap: 4 }}>
            <div onClick={function() { shiftMonth(side, 1); }}
              style={{ width: 36, height: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 6, background: T.grid, cursor: 'pointer', fontSize: 20, color: T.text, fontWeight: 600, userSelect: 'none' }}
              onMouseEnter={function(e) { e.currentTarget.style.background = T.gridHover; }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            {pickStart && <span style={{ fontSize: 14, color: T.primaryLight, fontWeight: 500 }}>
              {new Date(pickStart + 'T00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
