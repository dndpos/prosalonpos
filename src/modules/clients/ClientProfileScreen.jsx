...
// Package bridge removed — reading from packageStore directly
...
export default function ClientProfileScreen({ client, onBack, onBackToCalendar, onUpdateClient }) {
  var C = useTheme();
  var toast = useToast();
  var rbac = useRBAC();
  ...
  // ── Notes state (API-backed via client.notes JSON string) ──
  function parseNotes(raw) {
    ...
    try { var arr = JSON.parse(raw); return Array.isArray(arr) ? arr : []; }
      ...
      if (typeof raw === 'string' && raw.trim()) {
        return [{ id: 'legacy-1', content: raw.trim(), staff_name: '', created_at: client.created_at || new Date().toISOString() }];
      }
      return [];
    }
  }
  var [notes, setNotes] = useState(function() {
    return parseNotes(client.notes).sort(function(a, b) { return new Date(b.created_at) - new Date(a.created_at); });
  });
  ...
  // ── Package data — fetch from store directly ──
  var _pkgStore = usePackageStore(function(s) { return { fetch: s.fetchClientPackages, pkgs: s.clientPackages, items: s.clientPackageItems }; });
  var [profilePkgs, setProfilePkgs] = useState([]);
  var [profilePkgItems, setProfilePkgItems] = useState([]);
  useEffect(function() {
    if (client && client.id) {
      _pkgStore.fetch(client.id).then(function() {
        var st = usePackageStore.getState();
        setProfilePkgs(st.clientPackages || []);
        setProfilePkgItems(st.clientPackageItems || []);
      });
    }
  }, [client.id]);
  var clientPackages = profilePkgs;
  var clientPkgItems = useMemo(function() {
    var all = [];
    clientPackages.forEach(function(cp) {
      var items = profilePkgItems.filter(function(cpi) { return cpi.client_package_id === cp.id && cpi.remaining > 0; });
      items.forEach(function(cpi) { all.push({ ...cpi, packageName: cp.package_name, packageId: cp.id }); });
    });
    return all;
  }, [clientPackages, profilePkgItems]);
  ...
  var _fetchMembership = useMembershipStore(function(s) { return s.fetchClientMembership; });
  var [clientMembershipData, setClientMembershipData] = useState(null);
  useEffect(function() {
    if (client && client.id) {
      _fetchMembership(client.id).then(function(mem) {
        if (mem && mem.plan) {
          var renewal = mem.next_billing ? new Date(mem.next_billing).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 'N/A';
          setClientMembershipData({ plan: mem.plan.name, status: mem.status, renewal: renewal });
        } else { setClientMembershipData(null); }
      });
    }
  }, [client.id]);
  ...
  async function handleAddNote(text) {
    var newNote = {
      id: 'cn-' + Date.now(),
      client_id: client.id,
      staff_name: rbac.currentStaff ? rbac.currentStaff.display_name : 'Staff',
      content: text,
    ...
    setNotes(updated);
    try {
      await updateClient(client.id, { notes: JSON.stringify(updated) });
    } catch (err) {
      console.warn('[ClientProfile] Failed to save note:', err.message);
    }
  }
  ...
  async function handleDeleteNote(noteId) {
    ...
    try {
      ...
      console.warn('[ClientProfile] Failed to delete note:', err.message);
    }
  }
  ...
  var financials = useMemo(function() {
    ...
    return {
      ...
      gift_cards: [],
      membership: clientMembershipData,
  ...
  }, [client.id, closedTickets, clientMembershipData]);
  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', background: C.chrome, fontFamily: 'Inter,system-ui,sans-serif', overflow: 'hidden' }}>
      ...
      <div style={{ height: 52, background: C.chromeDark, display: 'flex', alignItems: 'center', padding: '0 20px', gap: 12, borderBottom: '1px solid ' + C.borderLight, flexShrink: 0 }}>
        <div onClick={onBackToCalendar || onBack} style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#0E2E3D', border: '1px solid #1A4A5C', color: '#7EC8E3', fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', padding: '7px 16px', borderRadius: 6, minHeight: 36, userSelect: 'none', transition: 'background 150ms' }}
            ...
            {activeTab === 'packages' && (
              <div>
                ...
                {clientPackages.map(function(cp) {
                  var cpItems = profilePkgItems.filter(function(cpi) { return cpi.client_package_id === cp.id; });
                  var redemptions = []; // TODO: fetch real redemption history from server
