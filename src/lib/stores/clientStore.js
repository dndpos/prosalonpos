...
var useClientStore = create(function(set, get) {
  return {
    ...
    addNote: async function(clientId, noteText, staffName) {
      try {
        var client = get().getClientById(clientId);
        var existing = [];
        if (client && client.notes) {
          try { existing = JSON.parse(client.notes); } catch(e) { existing = []; }
          if (!Array.isArray(existing)) existing = [];
        }
        var newNote = { id: 'cn-' + Date.now(), content: noteText, staff_name: staffName || 'Staff', created_at: new Date().toISOString() };
        var updated = [newNote].concat(existing);
        await get().updateClient(clientId, { notes: JSON.stringify(updated) });
      } catch (err) {
